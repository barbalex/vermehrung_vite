...
const createNew = async ({ node, store }) => {
  ...
  if (table === 'lieferung' && parentTable === 'kultur') {
    ...
    const kultur = await dexie.kulturs.get(parentId)
  ...
  if (table === 'lieferung' && parentTable === 'sammlung') {
    ...
    const sammlung = await dexie.sammlungs.get(parentId)
    additionalValuesToSet.art_id = sammlung?.art_id
  }
  if (table === 'lieferung' && parentTable === 'sammel_lieferung') {
    const sammelLieferung = await dexie.sammel_lieferungs.get(parentId)

...
export class Zaehlung implements IZaehlung {
  ...
  async edit({ field, value, store }) {
    ...
    // set all indexable boolean fields
    if (['_deleted', 'prognose'].includes(field)) {
      storeUpdate[`${field}_indexable`] = [undefined, null].includes(value)
        ? value
        : value
        ? 1
...
export class Person implements IPerson {
  ...
  async edit({ field, value, store }) {
    ...
    // set all indexable boolean fields
    if (
      ['_deleted', 'kein_email', 'kommerziell', 'info', 'aktiv'].includes(field)
    ) {
      storeUpdate[`${field}_indexable`] = [undefined, null].includes(value)
        ? value
        : value
...
export class Event implements IEvent {
  ...
  async edit({ field, value, store }) {
    ...
    // set all indexable boolean fields
    if (['_deleted', 'geplant'].includes(field)) {
      storeUpdate[`${field}_indexable`] = [undefined, null].includes(value)
        ? value
        : value
        ? 1
...
export class MySubClassedDexie extends Dexie {
  ...
  constructor() {
    ...
    this.version(1).stores({
      // TODO: set indexes
      herkunft: 'id, _deleted_indexable',
      sammlung: 'id, _deleted_indexable',
      lieferung: 'id, _deleted_indexable',
      art: 'id, _deleted_indexable',
      ae_art: 'id',
      garten: 'id, _deleted_indexable',
      kultur: 'id, _deleted_indexable',
      teilkultur: 'id, _deleted_indexable',
      zaehlung: 'id, _deleted_indexable',
      teilzaehlung: 'id, _deleted_indexable',
